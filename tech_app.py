...
nomalized_return=np.log(data['Adj Close']/data['Adj Close'].iloc[0])
...
dfr['y_unscaled'] = data['Adj Close']
dfr['y_pred_unscaled'] = np.exp(dfr['y_pred']) * data['Adj Close'].iloc[0]

data_len = len(data)
data['Number'] = np.arange(data_len)+1
df_high = data.copy()
df_low = data.copy()
...
data['Uptrend'] = slope * data['Number'] + intercept

slope, intercept, r_value, p_value, std_err = linregress(x=df_low['Number'], y=df_low['Close'])
data['Downtrend'] = slope * data['Number'] + intercept

fig6 = go.Figure(data=[go.Candlestick(x=data.index,
                open=data['Open'],
                high=data['High'],
                low=data['Low'],
...
fig6.add_trace(go.Scatter(x=data.index, y=dfr['y_pred_unscaled'], name='Regression',
                          line = dict(color='blue', width=2)))

fig6.add_trace(go.Scatter(x=data.index, y=data['Uptrend'], name='Resistance',
                         line = dict(color='red', width=2)))

fig6.add_trace(go.Scatter(x=data.index, y=data['Downtrend'], name='Support',
                         line = dict(color='green', width=2)))
...
for df in dfs:
    fig7.add_traces(go.Scatter(x=df.index, y = df['Leading_Span_A'],
                              line = dict(color='rgba(0,0,0,0)')))
    
    fig7.add_traces(go.Scatter(x=df.index, y = df['Leading_Span_B'],
                              line = dict(color='rgba(0,0,0,0)'),
                              fill='tonexty', 
...
fig7.add_trace(baseline)
fig7.add_trace(conversion)
fig7.add_trace(lagging)
fig7.add_trace(span_a)
fig7.add_trace(span_b)
...
fig7.update_layout(height=600, width=800, 
                  showlegend=True, title = f'{ticker.upper()} Ichimoku Cloud')
